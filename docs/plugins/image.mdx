...
<Warning>
  **Required Configuration**
  ...
  You **must** configure the `upload` and `delete` options. Without these options, you'll see an error when trying to use the Image plugin.
...
<AccordionGroup>
  ...
  <Accordion title="Error: Delete options are not configured">
    This error occurs when you're using a theme but haven't configured the `delete` option.
