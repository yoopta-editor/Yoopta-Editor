...
2. **SortableBlock** — Wrapper for each block that makes it sortable
3. **DragHandle** — The draggable trigger element
...
| Prop | Type | Description |
|------|------|-------------|
| `blockId` | `string` | Block ID this handle controls |
        ...
        <FloatingBlockActions>
          {({ blockId }) => (
            <>
              ...
              {/* Use DragHandle inside FloatingBlockActions */}
...
function Editor({ readOnly }) {
  ...
  const renderBlock = useCallback(({ children, blockId }: RenderBlockProps) => (
    <SortableBlock id={blockId} disabled={readOnly}>
      {!readOnly && (
        <DragHandle blockId={blockId} disabled={readOnly}>
          <GripVertical size={16} />
        </DragHandle>
      )}
      {children}
...
:root {
  /* Drag handle */
  --yoo-block-dnd-handle-opacity: 0;
  --yoo-block-dnd-handle-hover-opacity: 1;
  --yoo-block-dnd-handle-color: var(--yoopta-ui-muted-foreground);

  /* Drop indicator */
  --yoo-block-dnd-indicator-color: #3b82f6;
  --yoo-block-dnd-indicator-height: 2px;

  /* Dragging state */
  --yoo-block-dnd-dragging-opacity: 0.5;
  --yoo-block-dnd-dragging-bg: var(--yoopta-ui-accent);
}
```
...
```tsx
<SortableBlock id={blockId} className="group relative">
  <DragHandle
    blockId={blockId}
...
<AccordionGroup>
  <Accordion title="Memoize renderBlock">
    ...
    ```tsx
    const renderBlock = useCallback(({ children, blockId }: RenderBlockProps) => (
      <SortableBlock id={blockId}>{children}</SortableBlock>
    ), []);
    ```
  </Accordion>
  ...
  <Accordion title="Handle read-only mode">
    ...
    <SortableBlock id={blockId} disabled={isReadOnly}>
