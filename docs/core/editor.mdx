...
<Warning>
  Always wrap `createYooptaEditor()` in `useMemo` (with stable `plugins`/`marks`) to prevent
...
<ParamField
  ...
  Called when content changes. Use it to sync `value` to your state (e.g. for saving or controlled
  usage).
</ParamField>
...
<ParamField path="placeholder" type="string">
  Global placeholder text shown when the editor has a single empty block. If a plugin defines its
  own element-level placeholder, the element-level placeholder takes priority in multi-block
  editors. See [Placeholders](#placeholders) for details.
</ParamField>
...
export default function MyEditor() {
  ...
  return (
    <YooptaEditor editor={editor} onChange={handleChange} placeholder="Start typing..." autoFocus />
  );
}
```
