...
Themes use helper functions like `withShadcnUI()` to automatically apply styled components to your plugins.
...
**Installation:**
```bash
npm install @yoopta/editor slate slate-react
```
...
Tools provide UI components for editor interactions:
...
- **`@yoopta/link-tool`** - Link insertion tool
...
const plugins = withShadcnUI([Paragraph]);
