...
**Block Operations** (also available via `Blocks` namespace):
...
**Element Operations** (also available via `Elements` namespace):
...
```typescript
import { Blocks, Elements, Marks } from '@yoopta/editor';
...
Marks.update(editor, { type: 'bold', value: true, at: [0, 1] })
```
